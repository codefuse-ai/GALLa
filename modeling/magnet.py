...
from utils import print_rank_0, print_highlight, print_rank_0_highlight
from typing import Optional
import torch.nn.functional as F
from torch_geometric_signed_directed.nn import complex_relu_layer
    ...
    def forward(
        ...
        if self.weight.size(0) > 1:
            Tx_1_real_real = self.propagate(
                edge_index_real, x=x_real, norm=norm_real, size=None).to(self.weight[1].dtype)
            out_real_real = out_real_real + \
                torch.matmul(Tx_1_real_real, self.weight[1])
            Tx_1_imag_imag = self.propagate(
                edge_index_imag, x=x_imag, norm=norm_imag, size=None).to(self.weight[1].dtype)
...
class MagNet(nn.Module):
    ...
    Args:
        ...
        q (float, optional): Initial value of the phase parameter, 0 <= q <= 0.25. Default: 0.25.
        output_dim (int, optional): output dimension.  Default: 2.
    ...
    def __init__(self, num_features: int, hidden: int = 2, q: float = 0.25, K: int = 2, output_dim: int = 2,
        ...
        self.Conv = nn.Conv1d(2*hidden, output_dim, kernel_size=1)
